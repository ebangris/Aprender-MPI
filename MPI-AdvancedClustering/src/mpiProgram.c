...
int main ( int argc, char* argv[] )
{
  ...
  int *matrixOp1=NULL;//Se debe definir aqui para que la matrix solo se cree inicialmente en el nodo Root
  ...
  if(p_id==0)
  {
	...
	////////////////Send a msg to each son node////////////////////////////////////////
  	for(int i=0;i<ceil(log2(p));i++){
		...
		MPI_Send ((int *)malloc(sizeof(int)*(currNumRows/2)), 1,MPI_INT, sonNode, MSGTAG, MPI_COMM_WORLD);
	...
	for(int i=0 ;i<ceil(log2(p));i++){ // mayor entero, logaritnmo base 2 la mitad de la matriz
		int sonNode=(int)(pow(2,i)+p_id);	
		int lenMsg=(pow(2,floor(log2(p_id))))+N;
		int antLenMsg=lenMsg;
		if(lenMsg%2==0)
			lenMsg=lenMsg/2;
		else
			if(lenMsg>N)
  ...
  else if((int)(floor(log2(p_id))+1)<(int)(ceil(log2(p))))
  {
	...
	int lenMsg=(pow(2,floor(log2(p_id))))+N;//se calcula el tamaño del mensaje a recibir
